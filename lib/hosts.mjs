...
export const HOSTS = {
  claude: {
    ...
    command({ model, effort, schema, mode, shell }) {
      ...
      // readonly still runs commands (tests, git diff) — it only loses the write tools.
      if (mode === 'readonly') args.push('--allowedTools', 'Bash,Read,Grep,Glob,WebFetch,WebSearch')
  ...
  codex: {
    ...
    command({ model, effort, schemaFile, outFile, mode, extraDirs = [] }) {
    ...
    parse({ stdout, outFile }) {
      ...
      for (const line of stdout.split(/\r?\n/)) {
        ...
        try {
          ...
          if (e.type === 'turn.completed') outputTokens += e.usage?.output_tokens || 0
          if (e.type === 'turn.failed' || e.type === 'error') failure = e.error?.message || e.message || 'codex turn failed'
  ...
  opencode: {
    ...
    parse({ stdout }) {
      ...
      if (!lastMessage && failure) throw new Error(String(failure).slice(0, 300))
