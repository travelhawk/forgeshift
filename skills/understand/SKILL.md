...
Establish the target product: normally the folder you started your agent in (cd into it if
needed; ambiguous → ask). **Refuse to map the forge plugin repo itself** — if the target
holds a `.claude-plugin/plugin.json`, or `workflows/` next to `agents/forge-*.md`, that's the forge
...
`FORGE_HOME="${FORGE_HOME:-${CLAUDE_PLUGIN_ROOT:-$(forge-home 2>/dev/null || ls -d ~/.claude/plugins/cache/forge/forge/*/ | sort -V | tail -1)}}"`): a mapper's job *is* the broad read,
