...
manifest/package.json) — normally the folder you started your agent in; the gates run
...
`FORGE_HOME="${FORGE_HOME:-${CLAUDE_PLUGIN_ROOT:-$(forge-home 2>/dev/null || ls -d ~/.claude/plugins/cache/forge/forge/*/ | sort -V | tail -1)}}"` — then read e.g.
...
Run the `release-gate` workflow, `$FORGE_HOME/workflows/release-gate.js` by absolute path —
Claude Code: the Workflow tool with that file as `scriptPath`; any other host: `node
"$FORGE_HOME/bin/forge-run.mjs" release-gate --args-file <json>` — with `args: {dir: "<absolute product root>", context:
...
2. Deploy per the deploy section of the matching playbook
   (`$FORGE_HOME/docs/playbooks/<type>.md`) or the project AGENTS.md deploy command.
