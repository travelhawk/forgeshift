...
1. Resolve the plugin home once: `FORGE_HOME="${FORGE_HOME:-${CLAUDE_PLUGIN_ROOT:-$(forge-home 2>/dev/null || ls -d ~/.claude/plugins/cache/forge/forge/*/ | sort -V | tail -1)}}"`. The
...
3. Invoke the workflow `$FORGE_HOME/workflows/deep-review.js` with
   ...
   to a JSON file and run `node "$FORGE_HOME/bin/forge-run.mjs" deep-review --args-file <file>`
