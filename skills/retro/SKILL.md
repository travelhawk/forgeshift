...
| A rule everyone kept re-explaining | `AGENTS.md` (and something stale comes OUT) |
...
Changes land in the local install (`FORGE_HOME="${FORGE_HOME:-${CLAUDE_PLUGIN_ROOT:-$(forge-home 2>/dev/null || ls -d ~/.claude/plugins/cache/forge/forge/*/ | sort -V | tail -1)}}"`), so
...
- **A trap belongs in the harness only if it recurs across products.** One that is real but
  ...
  **that product's** `AGENTS.md`, not here. Ask per item: would this still be true for a CLI
