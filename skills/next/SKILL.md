...
- Harness assets (references, RISK-TIERS) ship with the plugin, not the product. Resolve
  their home once and reuse it: `FORGE_HOME="${FORGE_HOME:-${CLAUDE_PLUGIN_ROOT:-$(forge-home 2>/dev/null || ls -d ~/.claude/plugins/cache/forge/forge/*/ | sort -V | tail -1)}}"` — then
...
- **Pull in a reference if one fits.** If `$FORGE_HOME/references/INDEX.md` exists,
  ...
  whole folder (`$FORGE_HOME/AGENTS.md` → References).
