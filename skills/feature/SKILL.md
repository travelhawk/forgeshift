...
- Establish the target product (normally the directory you started your agent in; ambiguous →
  ask, then `cd`).
...
- **Read its risk tier** (`T?` on the feature's row). Untagged → classify by capability
  signal (`FORGE_HOME="${FORGE_HOME:-${CLAUDE_PLUGIN_ROOT:-$(forge-home 2>/dev/null || ls -d ~/.claude/plugins/cache/forge/forge/*/ | sort -V | tail -1)}}"`, then
