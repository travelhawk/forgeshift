---
name: adopt
description: Bring an EXISTING codebase under the harness - maps the code, reverse-engineers an as-built spec, seeds PROGRESS.md and a project AGENTS.md so the full lifecycle (/forge:feature, /forge:harden, /forge:ship) works on it. Use when the user points at a repo they already have instead of starting fresh.
argument-hint: "[path to the existing project]"
disable-model-invocation: true
---

# /forge:adopt — Existing codebase → harness contract
...
their home once in a Bash step and reuse it: `FORGE_HOME="${FORGE_HOME:-${CLAUDE_PLUGIN_ROOT:-$(forge-home 2>/dev/null || ls -d ~/.claude/plugins/cache/forge/forge/*/ | sort -V | tail -1)}}"`
— then read e.g. `$FORGE_HOME/templates/PROJECT-AGENTS.md`. Everything written below lands
in the adopted product (relative to its root), never under `$FORGE_HOME`.

## 1. Map

Run `/forge:understand` on the repo (from its root). It returns the architecture
...
1. `docs/SPEC.md` — as-built: what the product actually does today, its real users/
   ...
   capability signal — `$FORGE_HOME/docs/RISK-TIERS.md`) so later `/forge:feature`/`/forge:build`
   work on it is right-sized. **Mark it clearly at the top: "Reverse-engineered <date> —
   verify before relying on intent statements."** Unknowns go in Open Questions, not guesses.
2. `AGENTS.md` from `$FORGE_HOME/templates/PROJECT-AGENTS.md` — with the repo's REAL
   commands, verified by running them: install, test, dev, build. A command that fails
   gets documented as broken in Gotchas, not documented as working. Beside it a one-line
   `CLAUDE.md` holding `@AGENTS.md` (Claude Code reads only that name) — unless the repo
   already has a `CLAUDE.md`; then leave it and add the `@AGENTS.md` line on top.
3. `PROGRESS.md` from `$FORGE_HOME/templates/PROGRESS.md` — feature list seeded from
...
If a reference matches this product type — consult `$FORGE_HOME/references/INDEX.md`
...
(`$FORGE_HOME/AGENTS.md` → References).
...
test runner wired" because every later /forge:feature depends on it.
...
baseline, and the recommended first move. The full lifecycle now works: `/forge:feature`,
`/forge:harden`, `/forge:ship` all anchor on the files just created.
...
**Next →** the recommended first move as a command: `/forge:feature F0` when tests were
missing (wire the runner first), else `/forge:feature <highest-value F#>` — or `/forge:harden`
if the adopted app is already exposed. Name the one move.
