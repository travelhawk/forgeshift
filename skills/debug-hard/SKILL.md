---
name: debug-hard
description: Escalate a stubborn bug to the hard-bug debugging specialist with a structured handoff - it runs on the session model, so put the session on your host's strongest model for the hardest ones when you have the quota. Use when a bug survived two fix attempts, reproduces intermittently, or looks impossible - instead of a third normal attempt.
...
model (switch the session to your host's strongest model first if you have the quota).
