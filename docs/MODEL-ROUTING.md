...
|---|---|---|---|---|---|---|
| Claude Fable 5 | `claude-fable-5` | 1M | 128K | $10.00 | $50.00 | **The Architect.** Deepest reasoning, longest horizon. |
| Claude Opus 4.8 | `claude-opus-4-8` | 1M | 128K | $5.00 | $25.00 | **The Builder.** Default for all real engineering work. |
...
| Work | Model | Why |
|---|---|---|
| Product spec, architecture, system design | **Fable 5** | Errors here are the most expensive of the whole project |
| Technology/stack decisions with trade-offs | **Fable 5** | One-shot decision, long consequences |
| Hard debugging (heisenbugs, race conditions, "impossible" states) | **Fable 5** | Needs sustained hypothesis-driven reasoning |
| Adversarial review verdicts / judge panels | **Fable 5** | Judgment quality is the whole point of the gate |
| Orchestrating a large multi-agent build | **Fable 5** | Long-horizon coherence across many workstreams |
...
| Security audit passes | **Opus 4.8** | Careful reading, moderate reasoning depth |
| UI implementation from a settled design | **Opus 4.8** | High output volume, frequent small judgments |
| Interactive daily driving (default session model) | **Opus 4.8** | Fast mode available (`/fast`) for tight loops |
...
1. **Escalate to Fable when Opus loops.** Two failed fix attempts on the same bug, or a
   review that keeps flip-flopping → hand the full context to Fable 5 (`/debug-hard` does
   this automatically). Don't burn a third Opus attempt.
2. **De-escalate after the plan exists.** Fable writes the spec; Sonnet-tier subagents
   execute it. Paying Fable rates for `npm install` and boilerplate is waste.
3. **Never downgrade a gate.** Review/judge verdicts stay on Fable even under cost
   pressure — a missed bug costs more than the tokens.
