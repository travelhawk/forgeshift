...
suite('workflows', () => {
  for (const f of workflows) {
    ...
    test(`${f}: carries the shared args guard verbatim`, () => {
      // A workflow body cannot import, so the guard is pasted into every script and
      // regenerated by scripts/sync-workflow-preamble.mjs. One copy drifting is how the
      // hosts start behaving differently, so the text is pinned, not just its effect.
      assert.ok(src.includes(ARGS_GUARD), 'run: node scripts/sync-workflow-preamble.mjs')
    })
