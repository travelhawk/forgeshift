---
name: adopt
description: Bring an EXISTING codebase under the harness - maps the code, reverse-engineers an as-built spec, seeds PROGRESS.md and a project CLAUDE.md so the full lifecycle (/forge:feature, /forge:harden, /forge:ship) works on it. Use when the user points at a repo they already have instead of starting fresh.
...
2. `CLAUDE.md` from `$FORGE_HOME/templates/PROJECT-CLAUDE.md` — with the repo's REAL
...
(`$FORGE_HOME/CLAUDE.md` → References).
