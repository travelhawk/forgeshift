---
name: next
description: Hand the next version of an existing product to forge in one command - clarify new feature ideas into right-sized, risk-tiered features, append them to the spec, then build the whole slice hands-off through the forge flow (waves of PRs, merge, deep-review finish). Use after a product is already kicked off/adopted and you have new ideas or a next-version scope. New product = /kickoff; existing un-spec'd code = /adopt; one feature now = /feature.
argument-hint: "[new feature ideas / next-version description - empty = you'll be asked]"
disable-model-invocation: true
---

# /next — Ideas → tiered features → forge builds them

The iteration sibling of `/kickoff`. Kickoff turns an idea into a spec'd, scaffolded
product; `/next` turns *new* ideas into the next slice of that product and hands them to
...
`/next` is for **new** ideas that become new features. If you only want to build the
*existing* unchecked backlog, that's plain `/forge`.
...
- Establish the target product (cd into it; ambiguous → ask — pre-gate, allowed). It must
  ...
  and redirect**: a brand-new product is `/kickoff`; existing code without a spec is
  `/adopt`. `/next` iterates a product that already exists.
- Baseline must hold or you build on sand (same rule as `/forge` §0): working tree clean,
...
- **Right-size** per the granularity rule: a feature is a slice of user value
  ...
  version rather than cramming (`docs/RISK-TIERS.md` sizing; the spec's V-cap logic).
- **Tier each** by capability signal (`docs/RISK-TIERS.md`, ties break **upward**), each
...
- **Partition into waves** by predicted file footprint exactly as `/forge` §2 does
...
- **Proposed new features**: feature → version → tier → done-criteria. The tier column is
  your override point — bump any up or down here (`docs/RISK-TIERS.md`); ties break up.
- **The wave table**: feature → wave → tier → footprint → why it's parallel-safe.
- **Integration mode** (identical to `/forge` §3): auto-integrate (default) · review-PRs
...
## 5. Build & finish — the `/forge` flow, no second gate

Run `/forge`'s **Execute (§4)** and **Finish (§5 + §5b)** on exactly the approved new
...
(confirmed crit/high fixed in `/fix` discipline; unverified crit/high held ship-blocking)
→ visual walkthrough for UI products. **All of `/forge`'s stop conditions and hard rules
apply unchanged** — never force-push, never merge a failed feature, never weaken a test.

## 6. Report

Emit `/forge`'s report (§6) — feature → branch → PR → verdict → merged table, suite state
